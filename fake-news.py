...
def data_to_dataframe():
	dir1 = 'data/Fake.br-Corpus/size_normalized_texts/true'
	dir2 = 'data/Fake.br-Corpus/size_normalized_texts/fake'
	filelist1 = os.listdir(dir1) 
	filelist2 = os.listdir(dir2)

	tf_idf1=pd.DataFrame(columns=['Text'])
	for item in filelist1:
		with io.open(dir1+'/'+item, 'r', encoding='utf-8') as myfile:
			...
			tf_idf1=tf_idf1.append({'Text' : data},ignore_index=True)

	
	tf_idf2=pd.DataFrame(columns=['Text'])
	for item in filelist2:
		with io.open(dir2+'/'+item, 'r', encoding='utf-8') as myfile:
    			data=myfile.read()
			data = normalize('NFKD', data).encode('ASCII', 'ignore').decode('ASCII')
			data=pre_process(data)
			tf_idf2=tf_idf2.append({'Text' : data},ignore_index=True)
...
def SVM(X_train,y_train,X_test,y_test,stopwords):
	...
	print("Best parameters for SVM with SGD:")
